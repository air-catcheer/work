...
struct ThreadData {
    int clientSocket; // Структура для передачи данных потоку обработки клиента, содержит сокет клиента
};
...
void* HandleClient(void* data) {
    // Функция, которая обрабатывает взаимодействие с одним клиентом в отдельном потоке
...
void* ServerInputThread(void*) {
    // Функция, которая ждет ввода с клавиатуры и отправляет сообщения всем клиентам
    std::string input;
    while (true) {
